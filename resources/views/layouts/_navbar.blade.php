...
<header class="header">
    <div class="logo-container">
        <a href="../" class="logo">
            <img src="{{ asset('hebert_admin/images/logo_swelab_horizontal2.jpg') }}"
                 style="max-height:40px;height:100%;width:auto;" alt="Logo Open Swelab"/>
        </a>
    ...
    <div class="header-right">
        ...
        <ul class="notifications">
            ...
            <li class="{{Request::is('home')?'active':''}}" title="New Lab Result">
                <a href="{{route('home')}}" class="dropdown-toggle notification-icon"><i
                    class="fa fa-play-circle-o"></i></a>
            </li>

            @if (!Auth::guest())
            <li class="{{Request::is('result*')?'active':''}}" title="Lab Result">
                <a href="{{route('result.summary.index')}}" class="dropdown-toggle notification-icon"><i
            ...
            <li class="dropdown" title="Report">
                ...
                <ul class="dropdown-menu" role="menu">
                    <li class="{{Request::is('report/summary*')?'active':''}}"><a
                        href="{{route('report.summary.index')}}">Rekapitulasi</a></li>
                    <li class="{{Request::is('report/visitByRoom*')?'active':''}}"><a
                        href="{{route('report.visitByRoom.index')}}">Kunjungan Per Ruang</a></li>
                </ul>
            </li>
            @if(Auth::user()->roles == 'admin' || Auth::user()->roles == 'superadmin')
            <li title="Admin Panel">
                <a href="{{ route('settings.dashboard.index') }}" class="dropdown-toggle notification-icon"><i
                    class="fa fa-bars"></i></a>
            </li>
            @endif
            @endif
            <li class="{{Request::is('dashboard')?'active':''}}" title="Dashboard">
                <a href="{{route('dashboard')}}" class="dropdown-toggle notification-icon"><i
